...
public class QafManagerGeneratorBean extends QafManagerGenerator{

	public QafManagerGeneratorBean() {
	}
	...
	public void generisiAction(ActionEvent event) {
		...
		int kreiranoObjekata = izabranoEntiteta * 2 + 3;
		FacesContext context = FacesContext.getCurrentInstance();
		FacesMessage message = new FacesMessage();
		if(success){
			message.setSeverity(FacesMessage.SEVERITY_INFO);
			message.setSummary("Uspesno je kreirano " + kreiranoObjekata
					+ " klasa u paketu " + getManagerPackage() + " i " + izabranoEntiteta
					+ " klasa u paketu " + getSupportPackage());
		} else {
			message.setSeverity(FacesMessage.SEVERITY_ERROR);
			message.setSummary("Doslo je do greske prilikom generisanja klasa!");
		}
		context.addMessage("managerGeneratorForm", message);
	}
